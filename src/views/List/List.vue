<template>
  <header>
    <Head></Head>
  </header>
  <!-- 过滤 pokemon -->
  <div class="filter-part">
    <el-select multiple v-model="selectTypes" class="m-2 type-select" placeholder="Select Types" size="middle">
      <el-option
        v-for="item in typesValue"
        :key="item.value"
        :label="item.label"
        :value="item.value"
      />
    </el-select>
    <el-input
      v-model="searchPokemonName"
      class="w-50 m-2 name-input"
      placeholder="Search Name"
    >
      <template #prefix>
        <svg t="1663161164826" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1634" width="40" height="40"><path d="M533.36 513.91m-389.4 0a389.4 389.4 0 1 0 778.8 0 389.4 389.4 0 1 0-778.8 0Z" fill="#BCE8C4" p-id="1635"></path><path d="M517.94 932.9a419.07 419.07 0 0 1-415.63-365.68 8.29 8.29 0 0 1 16.45-2.09c25.43 200.21 197 351.18 399.18 351.18 221.89 0 402.41-180.52 402.41-402.41a401.78 401.78 0 0 0-21.29-129.5 8.29 8.29 0 1 1 15.7-5.33A418.87 418.87 0 0 1 517.94 932.9zM171.37 301.71a8.29 8.29 0 0 1-7-12.75 419.28 419.28 0 0 1 552.27-144 8.29 8.29 0 1 1-7.88 14.59 403 403 0 0 0-190.84-48c-138.22 0-265.16 69.67-339.56 186.38a8.29 8.29 0 0 1-6.99 3.78z" fill="" p-id="1636"></path><path d="M349.63 457s0-37-90.81-71.81C158.95 357.4 47 327.4 35 337.64 31.2 347.16 62.41 471.75 228 505c64.11 12.88 113.26 18.39 113.26 18.39s-2 101.91 7.76 141.43c13.61 54.83 68.72 133.47 160.86 137.32s176.88-6.3 210.96-28.14 75.06-109.5 81.69-134.51 6.85-48-6.41-66.24-36-56.74-40.71-69.24-15.15-56.74-28.4-69.24 70.12-71.57 107.88-106.5c60.68-56.16 111.62-186.12 95.53-197.66s-143.91 51.24-199.08 129.27c-39.59 56-82.89 124.94-82.89 124.94s-74.22-22.68-144.91-3.45S358.47 446.7 349.63 457z" fill="#FFD822" p-id="1637"></path><path d="M519.53 840.58q-15.77 0-31.78-0.66c-58.66-2.41-101-35.45-126.15-62.75-29.61-32.11-48.17-70-53.43-93.27-8.66-38.35 4.32-126.54 8.6-153.21-22.15 2.26-87.37 7.06-146.93-6.93-51.31-12.05-91.3-58.08-115.84-94.57-26.13-38.93-42.81-79.19-42.81-87.33a8.2 8.2 0 0 1 3.39-6.6c2.7-2.06 9.87-7.54 65.68 1.57 36.34 5.93 83.42 16.6 132.57 30.05 90.14 24.66 126.91 60.5 138.45 74.62 14.2-13.71 58.61-51.58 133.78-71.71 70.2-18.8 138-6.81 155.46-3.11 9.65-16.15 50-82.27 93.3-132.62 23.45-27.28 64.79-58 110.59-82.29 19.74-10.45 39-18.88 54.16-23.73 23.78-7.6 31.67-4.85 35.54-2.12 3 2.14 4.91 5.63 5.57 10.36 4.47 32.16-49.8 137.6-82.66 178.25-16.22 20.07-45 42.94-70.33 63.11-14 11.13-27.2 21.64-36.77 30.44-11.74 10.79-13.72 15.24-14.05 16.55 12.23 11.84 21.42 41.69 28.15 63.56 1.9 6.19 3.55 11.53 4.75 14.66 5 13 30.18 55.1 44.92 75.09 16.53 22.43 8.6 51.87 0.93 80.34-4.22 15.66-18.9 46.39-34.92 73.07-14.77 24.6-37 56.86-59.27 70.86-20.14 12.68-52.09 23-92.38 29.84-33.18 5.65-70.21 8.53-108.52 8.53z m-188.7-323.32l-1.34 7.74c-0.2 1.14-19.52 114-10 156.3 10.78 47.75 73.23 143 168.72 147 109.58 4.51 192.48-14.8 226-35.92 35.68-22.47 81.55-108.84 89.16-137.12 7.27-27 14.14-52.52 0.93-70.44-14.18-19.3-40.59-62.82-46.35-77.82-1.35-3.5-3-8.76-5-15.41-5.69-18.49-15.22-49.44-25.37-58.87a10.41 10.41 0 0 1-3.34-8.15c0.45-11.89 19.26-27.39 55.26-56 24.89-19.8 53.11-42.25 68.52-61.33 17.77-22 41.41-62.36 58.83-100.48 8.78-19.22 15.25-36.24 18.7-49.23 4.94-18.62 1.91-22.1 1.88-22.13-3.64-2.57-28.26 0.54-77.55 26.64-44.58 23.6-84.66 53.36-107.22 79.59-48.7 56.65-93.8 133.79-94.25 134.56l-2.2 3.81-4.22-1c-0.76-0.19-77.08-18.49-153.91 2.09C414.19 400.8 367 440.59 355 454.26l-5.27 6-4.08-6.86c-0.44-0.71-29.34-46.18-135.9-75.34-48-13.14-94.16-23.64-129.89-29.55C41 342.1 27.52 343.13 23.3 344c2.7 9.7 16.38 43.27 41.11 79.81 23.22 34.31 60.83 77.54 108.09 88.65 69.45 16.32 149.69 5.95 150.5 5.84z" fill="" p-id="1638"></path><path d="M817.58 608.3c0 32.36-12.48 59.91-15.23 59.37-23.83-4.67-42-28.76-42-57.77 0-21.8 13.4-45.74 28.6-55.83 6.27 12.93 24.88 25.1 28.63 54.23z" fill="#EC4304" p-id="1639"></path><path d="M802.43 674.05a6.83 6.83 0 0 1-1.31-0.12c-27.31-5.36-47.13-32.28-47.13-64 0-23 13.52-49.24 31.45-61.14l6.09-4 3.18 6.58c1.65 3.41 4.56 7.06 7.93 11.28 7.79 9.77 18.46 23.14 21.26 44.91l0.05 0.4v0.41c0 31.1-10.9 59.11-16.83 64a7.26 7.26 0 0 1-4.69 1.68z m-15.14-110.52c-11.71 10.91-20.54 30.26-20.54 46.38 0 24.25 13.83 44.93 33.36 50.67 3.75-7 11-27.79 11.09-51.86-2.42-18-11-28.71-18.53-38.18-1.89-2.39-3.73-4.69-5.38-7.01zM844.05 162.56s20.9 16.68 24.49 44.37c5.12 39.47-6.65 86.49-6.65 86.49L919 195.67l15.16-41.33s4.5-32.26-27.83-25-62.28 33.22-62.28 33.22z" fill="" p-id="1640"></path><path d="M906.56 155.57l-10.71 29.19-4.3 7.37A95.16 95.16 0 0 0 881 167.47a117.29 117.29 0 0 1 25.56-11.89m8.44-27.34a40 40 0 0 0-8.68 1.05c-32.33 7.21-62.28 33.27-62.28 33.27s20.9 16.68 24.49 44.37c5.12 39.47-6.65 86.49-6.65 86.49L919 195.67l15.16-41.33s3.64-26.1-19.15-26.1zM127.75 359c2.69 7.28-24.42 30.72-22.37 69.23a65.59 65.59 0 0 0 3.78 19 1.07 1.07 0 0 1-1.78 1.11C95.59 435.78 62.92 400.7 54 387c-10.95-16.81-19.22-44.93-19.22-44.93s90.29 9.66 92.97 16.93z" fill="" p-id="1641"></path><path d="M108.14 457.16a9.51 9.51 0 0 1-6.94-3C87.92 440 56.05 405.72 46.88 391.64c-11.5-17.65-19.9-46-20.25-47.17L23 332.27l12.64 1.35c0.23 0 23 2.47 45.8 5.78 13.5 2 24.41 3.84 32.43 5.59 14.78 3.23 19.88 5.81 21.82 11.08 2.33 6.32-1.23 12.32-6.15 20.63-6.73 11.35-16.9 28.5-15.7 51.09a57.22 57.22 0 0 0 3.27 16.49 9.61 9.61 0 0 1-9 12.89zM47 352c3.31 9.17 8.35 21.58 14.07 30.36s22.16 27.29 35.67 42C96.69 398.83 108 379.71 115 368c0.89-1.51 1.89-3.19 2.73-4.68C106 360 76 355.43 47 352z" fill="" p-id="1642"></path><path d="M421.68 691.54m-42.84 0a42.84 42.84 0 1 0 85.68 0 42.84 42.84 0 1 0-85.68 0Z" fill="#EC4304" p-id="1643"></path><path d="M412.42 751.22a56.35 56.35 0 1 1 56.35-56.35 56.42 56.42 0 0 1-56.35 56.35z m0-100.88a44.52 44.52 0 1 0 44.52 44.52 44.58 44.58 0 0 0-44.52-44.52z" fill="" p-id="1644"></path><path d="M485.86 566.18a42.12 42.12 0 0 1-42.12 42.12c-28 0-42.12-18.86-42.12-42.12s20.53-42.12 42.12-42.12c27.61 0 42.12 18.86 42.12 42.12z" fill="" p-id="1645"></path><path d="M450.406154 573.848655a20.71 22.65 11.41 1 0 8.961627-44.404721 20.71 22.65 11.41 1 0-8.961627 44.404721Z" fill="#FFFFFF" p-id="1646"></path><path d="M726.12 506.18c9.58 16.59 10 34.86-3.81 42.25-11.89 6.36-28.92-1.24-38.5-17.83s-9.64-34.51 3.81-42.25 28.92 1.24 38.5 17.83z" fill="" p-id="1647"></path><path d="M692.224037 517.55965a16.85 15.21 15.75 1 0 8.257219-29.277888 16.85 15.21 15.75 1 0-8.257219 29.277888Z" fill="#FFFFFF" p-id="1648"></path><path d="M538.27 618c7.29-1.39 16.58 1.7 24.11 2.08 8 0.4 16.17-0.12 23.52-3.62 8.82-4.2 21.68-22.54 31.65-21.57 6.79 0.66 12.14 8.58 19.35 10 5.37 1.06 11 0 16.09-2 4.84-1.93 9-5.06 13.67-7.26-0.07 2.46 0 5.11-0.19 7.6-0.73 7.51-1.28 15.08-2.29 22.56-2.08 15.47-3.1 31.2-5.1 46.75-2.63 20.42-6.47 40.8-13.14 60.32a212.26 212.26 0 0 1-12.81 29.38c-3.09 5.94-8.37 18.11-16.3 18.83-9.92 0.9-19.75-14-24.67-20.59a200.78 200.78 0 0 1-21.61-35.28c-9.23-19.82-15.43-40.87-20.87-62-2.58-10-5.25-19.94-7.93-29.92-1.32-5-2.64-10.35-3.48-15.28z" fill="#892727" p-id="1649"></path><path d="M615.95 786.88c-11.68 0-21.25-13.12-27.06-21.08l-1.38-1.89c-9.66-13-16.72-24.51-22.21-36.3-9.86-21.17-16.27-43.73-21.23-63-2.46-9.56-5.06-19.24-7.57-28.61l-0.34-1.25c-1.18-4.43-2.65-10.21-3.6-15.82a5.81 5.81 0 0 1 4.64-6.67c5.64-1.08 11.58-0.08 17.33 0.89a74.31 74.31 0 0 0 8.16 1.09c8.95 0.45 15.34-0.49 20.73-3.06 2.59-1.23 6.41-4.79 10.11-8.22 7.8-7.24 15.86-14.74 24.6-13.88 5.07 0.49 9.09 3.41 12.63 6 2.59 1.88 5 3.66 7.28 4.1 3.73 0.74 8.16 0.15 12.81-1.7a53.94 53.94 0 0 0 7.08-3.7c2-1.16 4-2.36 6.24-3.41a5.81 5.81 0 0 1 8.3 5.42v2.46c0 1.79 0 3.64-0.19 5.54-0.21 2.16-0.41 4.33-0.6 6.49-0.48 5.33-1 10.83-1.71 16.27-1.07 7.93-1.87 16.13-2.64 24.06-0.73 7.43-1.48 15.1-2.45 22.66-3.13 24.32-7.39 43.85-13.4 61.45a217.86 217.86 0 0 1-13.15 30.18c-0.3 0.58-0.63 1.23-1 1.91-3.6 7.14-9.61 19.08-20 20-0.45 0.05-0.92 0.07-1.38 0.07z m-70.63-163.33c0.69 3.05 1.44 5.94 2 8.19l0.33 1.25c2.52 9.39 5.13 19.11 7.6 28.72 4.83 18.76 11 40.68 20.51 61 5.15 11.05 11.82 21.94 21 34.27l1.45 2c3.67 5 12.29 16.83 18 16.3 3.92-0.36 8.62-9.7 10.63-13.68 0.37-0.74 0.72-1.42 1-2a206.6 206.6 0 0 0 12.46-28.57c5.75-16.85 9.84-35.65 12.87-59.18 1-7.38 1.69-15 2.41-22.31 0.79-8 1.6-16.33 2.69-24.47 0.7-5.19 1.18-10.57 1.65-15.77l0.32-3.48a46.77 46.77 0 0 1-5.25 2.52c-6.74 2.68-13.43 3.47-19.36 2.3-4.76-0.94-8.53-3.68-11.86-6.1-2.58-1.87-5-3.64-6.93-3.83-3.51-0.31-10.78 6.38-15.57 10.83-4.54 4.22-8.83 8.2-13 10.2-9.36 4.46-19.5 4.52-26.31 4.17a84.22 84.22 0 0 1-9.5-1.23c-2.32-0.5-4.81-0.92-7.14-1.13z" fill="" p-id="1650"></path><path d="M545.53 647.36s14.84-12.87 43.86-10c26.67 2.6 64.5 18.07 66.6 45.68 0.17 29.75-15.72 68-29.11 90.43s-23.28 0-38.71-18.46-28.17-60.39-31.34-68.68-11.3-38.97-11.3-38.97z" fill="#E27171" p-id="1651"></path><path d="M615.56 788.62c-9.11 0-16-9.18-23.93-19.77-2.52-3.37-5.13-6.84-7.91-10.16-13.73-16.43-24.89-48.84-30.25-64.42-0.9-2.61-1.61-4.68-2.07-5.87-3.23-8.48-11.15-38.29-11.49-39.55a5.81 5.81 0 0 1 1.81-5.88c0.68-0.59 17.08-14.46 48.23-11.43 15.46 1.51 32.24 6.83 44.88 14.23 16.53 9.68 25.85 22.4 26.94 36.79v0.41c0.17 29.68-14.91 68.32-29.93 93.45-4.72 7.9-9.83 11.89-15.6 12.19zM552.2 649.79c2.29 8.49 7.71 28.29 10.06 34.47 0.52 1.37 1.26 3.51 2.19 6.23 4.74 13.75 15.83 46 28.18 60.75 3 3.55 5.67 7.16 8.28 10.64 4.84 6.45 11.43 15.3 14.75 15.12 1.16-0.06 3.4-1.81 6.24-6.55 13.7-22.92 28.37-59.94 28.29-87.21-2-23.75-37-37.77-61.36-40.14-19.26-1.87-31.45 3.6-36.63 6.69z" fill="" p-id="1652"></path><path d="M601.521288 558.891965a5.13 13.02 81.9 1 0 25.780216-3.669068 5.13 13.02 81.9 1 0-25.780216 3.669068Z" fill="" p-id="1653"></path><path d="M609.44 566.5c-6.59 0-12.38-2-13.15-7.11-1-6.41 7.18-10.57 15.68-11.85a30.35 30.35 0 0 1 10.92 0.17c5.93 1.36 7.28 4.67 7.56 6.53 1 6.41-7.18 10.57-15.68 11.85a36 36 0 0 1-5.33 0.41z m-2.71-8.68a25.94 25.94 0 0 0 13.29-2 25.94 25.94 0 0 0-13.29 2z m15.83-1.25z" fill="" p-id="1654"></path></svg>
      </template>
    </el-input>
  </div>
  <!-- 关于 -->
  <div class="about" @click="showAbout()">
    <div class="about-border"></div>
    <div class="about-text">About Me</div>
    <div>
    </div>
  </div>
  <!-- pokemon 列表 -->
  <div id="List">
    <div class="border" v-for="pokemon in activePokemons" :key="pokemon.id" :style="{'--color': colors[pokemon.types[0].type.name]}">
      <div class="item">
        <div class="img-container">
          <img :src="pokemon.sprites.front_default" onerror="javascript:this.src='data:image/jpeg;base64,'">
        </div>
        <div class="id">#{{pokemon.id.toString().padStart(3, '0')}}</div>
        <h3>{{pokemon.name}}</h3>
        <span class="types" v-for="curType in pokemon.types">
          <small>{{curType.type.name}}</small>
        </span>
      </div>
      <div class="details">
        <h4>{{pokemon.name}}</h4>
        <button @click="showCard(pokemon)">More Details</button>
      </div>
    </div>
  </div>
  <el-pagination background layout="prev, pager, next" :total="total" :page-size="pageSize" v-model:current-page.sync="currentPage"/>
</template>

<script lang="ts">
  import { defineComponent, ref, reactive, toRefs, onBeforeMount, onMounted, watchEffect, computed, getCurrentInstance } from 'vue';
  import type { SelectProps } from 'ant-design-vue';
  import { getPokemonApi } from '../../service/api/index'
  import Head from '../../components/Head/Head.vue'

  export default defineComponent({
    components: {
      Head
    },
    setup() {
      // 不同种族 不同卡片颜色
      interface IColors {
        [propname: string]: string
      }

      const colors: IColors = reactive({
        fire: '#ff927d',
        grass: '#bdffa3',
        electric: '#ffcc33',
        water: '#77bbff',
        ground: '#f1dda0',
        rock: '#e1d08c',
        fairy: '#fbcbfb',
        poison: '#c689ba',
        bug: '#daec44',
        dragon: '#a194ff',
        psychic: '#ff9cc4',
        flying: '#99bbff',
        fighting: '#dd9988',
        normal: '#e1e1ff',
        ghost: '#9f9fec',
        steel: '#dfdfe1',
        ice: '#dbf6ff',
        dark: '#bda396',
      })
      
      // 获取宝可梦数据 写入pokemons
      const pokemons = reactive<IPokemon[]>([])

      async function getPokemon() {
        // 899
        for (let i = 1; i < 899; i++) {
          await getPokemonApi(i)
            .then((response) => {
              pokemons.push(response.data)
            })
        }
      }

      getPokemon()

      /* 搜索框 */
      // 搜索名字
      const searchPokemonName = ref<string>('')
      const searchPokemon = () => {
      }

      function filterPokemonName(pokemon: IPokemon) {
        return pokemon.name.indexOf(searchPokemonName.value) !== -1
      }

      /* 选择框 */
      // 选择种族
      const typesValue = ref<SelectProps['options']>([
        {
          value: 'fire',
          label: 'Fire'
        },
        {
          value: 'grass',
          label: 'Grass'
        },
        {
          value: 'electric',
          label: 'Electric',
        },
        {
          value: 'water',
          label: 'Water'
        },
        {
          value: 'ground',
          label: 'Ground'
        },
        {
          value: 'rock',
          label: 'Rock'
        },
        {
          value: 'fairy',
          label: 'Fairy'
        },
        {
          value: 'poison',
          label: 'Poison'
        },
        {
          value: 'bug',
          label: 'Bug'
        },
        {
          value: 'dragon',
          label: 'Dragon'
        },
        {
          value: 'psychic',
          label: 'Psychic'
        },
        {
          value: 'flying',
          label: 'Flying'
        },
        {
          value: 'fighting',
          label: 'Fighting'
        },
        {
          value: 'normal',
          label: 'Normal'
        },
        {
          value: 'ghost',
          label: 'Ghost'
        },
        {
          value: 'steel',
          label: 'Steel'
        },
        {
          value: 'ice',
          label: 'Ice'
        },
        {
          value: 'dark',
          label: 'Dark'
        }
      ])
      const selectTypes = ref<string[]>([])

      function filterPokemonType(pokemon: IPokemon) {
        // 如果没选 就全显示
        if (selectTypes.value.length === 0) return pokemon
        const { types } = pokemon

        // 只要宝可梦有这个种族就返回 true
        function filterType(selectType: string) {
          return types.some((typeSlot) => typeSlot.type.name === selectType)
        }

        // 宝可梦必须有所有被选中的种族, 才可以被激活(遍历每一个被选中的种族)
        return selectTypes.value.every(filterType)
      }

      /* 激活的 pokemon */
      const activePokemons = computed(() => {
        return pokemons.filter(filterPokemonType).filter(filterPokemonName).slice((currentPage.value-1)*pageSize.value, currentPage.value*pageSize.value)
      })
      
      const instance = getCurrentInstance()
      const _this= instance!.appContext.config.globalProperties
      const showCard = (pokemon: IPokemon) => {
        _this.$router.push({
          path: `/card/${pokemon.id}`,
        })
      }

      /* 关于 */
      const showAbout = () => {
        _this.$router.push({
          path: `/about`,
        })
      }
      
      /* 分页器 */
      const total = ref<number>(898)
      const pageSize = ref<number>(20)
      const currentPage = ref<number>(1)
      
      return {
        colors,
        pokemons,
        searchPokemonName,
        searchPokemon,
        showAbout,
        typesValue,
        selectTypes,
        activePokemons,
        showCard,
        total,
        pageSize,
        currentPage,
      }
    }
  })
  
</script>

<style scoped lang='less'>
  header {
    height: 10vw;
    margin-bottom: 1.5vw;
  }

  @tag-color: #e3eeff;
  .filter-part {
    display: flex;
    justify-content: center;
    align-items: center;
    .type-select {
      margin-right: 3%;
      :deep(.el-tag) {
        background-color: @tag-color;
      }
    }
    .name-input {
      width: 40%;
    }
  }
  .about {
    position: relative;
    top: -2vw;
    right: -75vw;
    width: 12vw;
    height: 2.5vw;
    line-height: 2.5vw;
    text-align: center;
    background-color: transparent;
    color: black;
    margin-left: 2.5vw;
    cursor: pointer;
    .about-text {
      position: absolute;
      left: 50%;
      top: 50%;
      transform: translate(-50%, -50%);
      font-weight: 600;
    }
    .about-border {
      // width: 100%;
      height: 100%;
      background-color: transparent;
      border: 2px solid;
      border-image: linear-gradient(45deg, rgb(184, 159, 218), rgb(15, 48, 194)) 1;
      clip-path: inset(0 100% 0 0);
      animation: clipPath 5s infinite linear;
    }
    @keyframes clipPath {
      0%{
        clip-path: inset(0 0 100% 0);
        filter: hue-rotate(0deg);
      }
      25% {
        clip-path: inset(0 100% 0 0);
      }
      50% {
        clip-path: inset(100% 0 0 0);
        filter: hue-rotate(360deg);
      }
      75% {
        clip-path: inset(0 0 0 100%);
      }
      100% {
        clip-path: inset(0 0 100% 0);
        filter: hue-rotate(0deg);
      }
    }
  }
  #List {
    display: flex;
    justify-content: center;
    align-items: center;
    flex-wrap: wrap;
  }
  .border {
    position: relative;
    margin: 15px;
    background: white;
    border-radius: 10px;
    padding: 8px;
    // 外阴影和内阴影
    box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.2),
                -2px -2px 5px rgba(0, 0, 0, 0.2) inset;
    &:hover, &:focus {
      animation: pulse 1s;
      // x 偏移量 | y 偏移量 | 阴影模糊半径 | 阴影扩散半径 | 阴影颜色
      box-shadow: 0 0 0 0.7vw transparent;
    }

    @keyframes pulse {
      0% {
        box-shadow: 0 0 0 0 var(--color);
      }
    }
  }
  .item { 
    width: 180px;
    height: 270px;
    border-radius: 10px;
    padding: 15px;
    text-align: center;
    background-color: var(--color);
  }
  .img-container {
    background-color: rgba(255, 255, 255, 0.6);
    border-radius: 50%;
    width: 120px;
    height: 120px;
    margin: auto;
  }
  img {
    width: 100%;
    margin-top: 0px;
    margin-left: 0px;
  }
  .id {
    font-size: 12px;
    margin: 15px auto;
    padding: 5px 10px;
    border-radius: 10px;
    width: fit-content;
    background: rgba(0, 0, 0, 0.1);
  }
  .types {
    font-weight: bold;
    border-radius: 10px;
    width: fit-content;
    margin: 15px auto;
    background: rgba(255, 255, 255, 0.4);
    padding: 5px 8px;
    margin: 0 5px;
  }
  .details {
    opacity: 0;
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    color: rgb(194, 168, 168);
    border-radius: 10px;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    transition: opacity 0.3s ease-in 0s;
    font-size: 20px;

    &:hover {
      background-image: url('./img/xiaozhi.jpeg');
      background-size: cover;
      background-position: center;
      opacity: 1;
    }

    button {
      color: #fff;
      background-color: #766a85;
      // font-family: Arial, Helvetica, sans-serif;
      font-size: 15px;
      font-weight: 500;
      text-align: center;
      transition: all 0.5s ease 0s;
      border: 0;
      &:hover {
        transform: scale(0.98);
        opacity: 0.9;
        background: #766a85;
      }
    }

    h4 {
      padding: 5px 10px 5px 10px;
      box-shadow: 0 0.3px 0.7px rgba(0, 0, 0, 0.126),
        0 0.9px 1.7px rgba(0, 0, 0, 0.179), 0 1.8px 3.5px rgba(0, 0, 0, 0.224),
        0 3.7px 7.3px rgba(0, 0, 0, 0.277), 0 10px 20px rgba(0, 0, 0, 0.4);
      backdrop-filter: blur(20px);
      transition: 0.5s ease;

      &:hover {
        box-shadow: 0 0.7px 1px rgba(0, 0, 0, 0.157),
          0 1.7px 2.6px rgba(0, 0, 0, 0.224), 0 3.5px 5.3px rgba(0, 0, 0, 0.28),
          0 7.3px 11px rgba(0, 0, 0, 0.346), 0 20px 30px rgba(0, 0, 0, 0.5);
      }
    }
  }

  #Card {
    position: fixed;
    top: 50vh;
    left: 50vw;
    width: 1000px;
    height: 600px;
    transform: translate(-50%, -50%);
    background-color: white;
    opacity: 0.8;
    border: 1px solid black;
    border-radius: 50px;
  }
  
  /deep/ .el-pagination, .is-background {
    display: flex;
    justify-content: center;
    margin-top: 1vw;
    padding: 0.5vw;
    background-color: #766a85;
    box-shadow: 10px 10px 5px #888888;
    opacity: 0.5;
    animation: changeColor 2s linear infinite;
    @keyframes changeColor {
      from {
        filter: hue-rotate(0)
      }
      to {
        filter: hue-rotate(360deg);
      }
    }
  }
  /deep/ .number {
    &:hover, :focus {
      box-shadow: 0 0.5vw 0.5vw -0.4vw;
      transform: translateY(-0.25vw);
    }
  }
</style>